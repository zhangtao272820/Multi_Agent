...
const checks: Array<[boolean, string]> = [
  ...
  [read('sidebar').includes('步骤进度（') || read('sidebar').includes('处理进度（'), 'sidebar keeps compact step progress'],
  [!read('sidebar').includes('ManagerSpecialistCards'), 'sidebar no specialist cards'],
  [read('thread').includes('mgr-task-board-in-thread'), 'thread hosts task board'],
  [read('thread').includes('ManagerSpecialistCards'), 'thread hosts specialist cards'],
  [read('page').includes('collectUserCreatedPlanAgents'), 'page uses plan-label collector'],
