...
const ORCH_BASE_RULES = [
  ...
  '【taskIntent】仅填 structured_query|document_retrieval|hybrid|action|chitchat|unknown（禁止自造词如 hybrid_multi_source_*）；structured_query→db；document_retrieval→rag；hybrid→多源拆句；对照 catalog（领域随库变）；Probe命中≠默认查库。',
  '【executionTopology】仅填 solo|parallel|hub（禁止 parallel_hub 等组合词）；solo=单专才；parallel=无依赖 fan-out；hub=有依赖 DAG。',
