...
const USER_DATA_REPORT_STRUCTURE = [
  ...
  '2) ### 关键发现：3～5 条要点；异常/偏高/偏低优先，正常项可一句带过。可用 **加粗** 标数字。',
  '3) ### 详细说明：异常项解读；多项对照用 Markdown 表（指标 | 测值 | 参考 | 状态）；状态用「正常 / 略高 / 略低 / 需关注」。',
...
const USER_DATA_STANDARD_STRUCTURE = [
  ...
  '2) 需要展开时用 2～4 个 ### 小标题分段；对比/推荐优先 Markdown 表。',
  '3) 正文用 [1][2] 角标；末段一句小结或建议即可。',
  '4) 简单事实题 2～8 句答完，勿硬套多级标题。',
  '禁止执行摘要、管线腔、库表审计字段。'
].join('\n')

/** lite 档：短确认 */
const USER_DATA_LITE_STRUCTURE = [
  '结构（lite 档）：2～8 句直接说清结果；无强制 ###；无附录；无执行摘要。',
...
const PROFILE_CHART_REPORT = [
  ...
  '本任务含图表/报告附属输出：按复杂度充分展开，写清关键数字、对比结论与图表含义；附属块仅作补充，正文不得缩成一句空摘要。',
  '已有 visualize/report 子输出时：正文仍须覆盖关键结论与数字；与 <!--REPORT--> 对同一事实须一致，但勿把附录整段粘进正文。',
...
const PROFILE_CHART_REPORT_WITH_ADMIN = [
  SYNTH_PACK_MARKERS.chart_report,
  '本任务含图表与 admin 等多步：写清①关键数字与口径②对比结论③图表含义④admin 实际执行结果。正文勿因「另有报告」而缩短。',
...
const PROFILE_GUI_ONLY = [
  ...
  '有 GUI 时：正文基于 GUI 子步骤（页面操作结论、抽取到的标题/链接/文本）；禁止声称「系统限制网页操作」。',
  '若 agentResult.ok=false 或 navigation_unverified / incomplete：如实说明失败原因；禁止编造成功标题。'
].join('\n')

const PROFILE_GUI_WITH_DB = [
  SYNTH_PACK_MARKERS.gui_web,
  '同时有 GUI 与 DB：库内数字以 DB 为准；GUI 仅补充页面可见信息。',
  '有 GUI 时：正文必须基于 GUI 子步骤输出；禁止声称「系统限制网页操作」。'
].join('\n')

const PROFILE_CHAT_WEB = [
  SYNTH_PACK_MARKERS.gui_web,
  '联网/知识讲解（DeepSeek 风）：短开篇（2～3 句）→ 2～4 个 ### 分段 → 列表或对照表 → 末段一句小结。',
  '对比/推荐优先 Markdown 表；正文 [1][2] 角标；信息不足时说明缺口。',
  '简单事实题：2～8 句直接答完。'
].join('\n')
