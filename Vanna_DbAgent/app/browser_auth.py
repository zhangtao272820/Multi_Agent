"""ClawHive 浏览器 JWT / internal token 门禁（与 shared/clawhive_jwt.py、AI_admin browser_auth 对齐）。"""
...
def _internal_ok(headers: dict[str, str]) -> bool:
    ...
    got = (
        ...
        or headers.get("x-agent-service-token")
        or ""
    ).strip()
...
def install_auth_config_route(app) -> None:
    ...
    def _auth_config() -> dict[str, Any]:
        return {
            "clawhiveAuthUrl": clawhive_public_auth_url(),
            "browserAuth": browser_auth_enabled(),
        }
